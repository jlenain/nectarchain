...
def ComputePedestalEvolution(run, path):
    ...
    for evt in tqdm(data):
        pedestal_times.append(evt.tel[0].pedestal.startTime.datetime)
        pedestal_positions.append(evt.tel[0].pedestal.pos.copy())
        pedestal_widths.append(evt.tel[0].pedestal.width.copy())
        pedestal_pos_mean.append(evt.tel[0].pedestal.pos.mean(axis=2))
        pedestal_width_mean.append(evt.tel[0].pedestal.width.mean(axis=2))
        if use_db:
            # assume that the shape is [channel,pixel,slices]
            temperatures1_entry = np.zeros(evt.tel[0].pedestal.width.shape[:-1])
            temperatures2_entry = np.zeros(evt.tel[0].pedestal.width.shape[:-1])
            for pix in range(evt.tel[0].pedestal.width.shape[1]):
