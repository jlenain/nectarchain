...
def FitPixel(hicharges, sigped=None, ped=None):
    ...
    pix_datas = hicharges
    bins = np.linspace(
        np.min(pix_datas) - 0.5,
        np.max(pix_datas) + 0.5,
        int(np.max(pix_datas) - np.min(pix_datas) + 2),
    )
...
def DoSPEFFFit(arglist):
    ...
    for evt in tqdm(data, total=nEvents):
        charges, times = SignalIntegration(
            evt.r0.tel[0].waveform,
            exclusion_mask=evt.mon.tel[0].pixel_status.hardware_failing_pixels,
            peakpositions=t0max,
        ...
        ped_stats.add(
            ped, validmask=~evt.mon.tel[0].pixel_status.hardware_failing_pixels
        )
    ...
    cam_illu = CameraDisplay(
        geometry=GetCamera(),
        image=ma.array(cam_illu_values, mask=cam_gain_values < 40.0),
        cmap="turbo",
    ...
    cam_ns = CameraDisplay(
        geometry=GetCamera(),
        image=ma.array(cam_ns_values, mask=cam_gain_values < 40.0),
        cmap="turbo",
    ...
    cam_pedw = CameraDisplay(
        geometry=GetCamera(),
        image=ma.array(cam_pedw_values, mask=cam_gain_values < 40.0),
        cmap="turbo",
    ...
    with open(outdataname, "w") as outfile:
        ...
        for p, f in pixelFitResult.items():
            if f.minuit_result.valid:
                outfile.write(
                    f"{p} {f.fit_parameters['Ped'].value} {f.fit_parameters['SigmaPed'].value} {f.fit_parameters['Ns'].value} {f.fit_parameters['Illu'].value} {f.fit_parameters['Gain'].value} {f.fit_parameters['SigmaGain'].value}\n"
                )
